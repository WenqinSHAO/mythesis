%*******************************************************
% Table of Contents
%*******************************************************
%\phantomsection
\refstepcounter{dummy}
\pdfbookmark[1]{\contentsname}{tableofcontents}
\setcounter{tocdepth}{2} % <-- 2 includes up to subsections in the ToC
\setcounter{secnumdepth}{3} % <-- 3 numbers up to subsubsections
\manualmark
\markboth{\spacedlowsmallcaps{\contentsname}}{\spacedlowsmallcaps{\contentsname}}
\tableofcontents 
\automark[section]{chapter}
\renewcommand{\chaptermark}[1]{\markboth{\spacedlowsmallcaps{#1}}{\spacedlowsmallcaps{#1}}}
\renewcommand{\sectionmark}[1]{\markright{\thesection\enspace\spacedlowsmallcaps{#1}}}
%*******************************************************
% List of Figures and of the Tables
%*******************************************************
\clearpage

\begingroup 
    \let\clearpage\relax
    \let\cleardoublepage\relax
    \let\cleardoublepage\relax
    %*******************************************************
    % List of Figures
    %*******************************************************    
    %\phantomsection 
    \refstepcounter{dummy}
    %\addcontentsline{toc}{chapter}{\listfigurename}
    \pdfbookmark[1]{\listfigurename}{lof}
    \listoffigures

    \vspace{8ex}
	\newpage
    %*******************************************************
    % List of Tables
    %*******************************************************
    %\phantomsection 
    \refstepcounter{dummy}
    %\addcontentsline{toc}{chapter}{\listtablename}
    \pdfbookmark[1]{\listtablename}{lot}
    \listoftables
        
    \vspace{8ex}
   \newpage
 
\iffalse   
    %*******************************************************
    % List of Listings
    %*******************************************************      
      %\phantomsection 
    \refstepcounter{dummy}
    %\addcontentsline{toc}{chapter}{\lstlistlistingname}
    \pdfbookmark[1]{\lstlistlistingname}{lol}
    \lstlistoflistings 

    \vspace{8ex}
\fi

      
    %*******************************************************
    % Acronyms
    %*******************************************************
    %\phantomsection 
    \refstepcounter{dummy}
    \pdfbookmark[1]{Acronyms}{acronyms}
    \markboth{\spacedlowsmallcaps{Acronyms}}{\spacedlowsmallcaps{Acronyms}}
    \chapter*{Acronyms}
    \begin{acronym}[ARIMA]
    		\acro{ACM}{Association for Computing Machinery}
    		\acro{NAT}{Network Address Translation}
    		\acro{LISP}{Locator/Identifier Separation Protocol}
    		\acro{SDN}{Software Defined Networking}
    		\acro{RIB}{Internet Routing Information Base}
    		\acro{FIB}{Forwarding Information Base}
    		\acro{TE}{Traffic Engineering}
    		\acro{IGP}{Interior Gateway Protocol}
    		\acro{BGP}{Border Gateway Protocol}
    		\acro{AS}{Autonomous System}
    		\acro{MED}{Multi-exit Discriminator}
    		\acro{RTT}{Round-Trip Time}
    		\acro{CDN}{Content Delivery Network}
    		\acro{LB}{Load Balancing}
		\acro{TSF}{Time Series Forecasting}
		\acro{ANN}{Artificial Neural Networks}
		\acro{ISP}{Internet Service Provider}
		\acro{CP}{Content Provider}
		\acro{HP}{Hosting Provider}
		\acro{IXP}{Internet Exchange Point}
    		\acro{IRP}{Intradomain Routing Pattern}
    		\acro{EWMA}{Exponentially Weighted Moving Average}
    		\acro{ARIMA}{Autoregressive integrated moving average}
    		\acro{PoP}{Point of Presence}
    		\acro{RIPE}{Réseaux IP Européens}
    		\acro{PAM}{Partitioning Around Medoids}
    		\acro{ASW}{Average Silhouette Width}
    		\acro{PCA}{Principal Component Analysis}
    		\acro{UDM}{User Defined Measurements}
    		\acro{DTW}{Dynamic Time Wrapping}
    		\acro{ED}{Eucilidean Distance}
    		\acro{NOC}{Network Operation Center}
    		\acro{NDA}{Non-disclosure Agreement}
    		\acro{DM}{Divergent Measurement}
    		\acro{FM}{Feature Measurement}
    		\acro{EXT}{Extension Link}
    		\acro{iEXT}{Impacted Extension Link}
    \end{acronym}                     
\endgroup
