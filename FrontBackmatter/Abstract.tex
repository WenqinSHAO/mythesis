...
Then, best route is chosen for each destination considering not alone policies but as well measurements. That is the main idea of measurement-based \acf{TE}.
...
We encounter difficulties in meaningfully clustering latency measurements. 
...
When paths toward these destinations traverse change causes, we can reasonably assume similar performance changes on these paths as well.
We developed a series of inference procedures to attribute the cause of latency changes to \acf{AS} or inter-AS links.
