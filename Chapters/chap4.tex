...
There are two approaches to labeled ground truth dataset: 1) artificially generated data; 2) real data with labels manually put on.
Real data is more representative of the Internet delay character.
It is thus the preferred choice.
However, it can only be labeled by humans with domain knowledge in absence of systematic automation.
...
Therefore, it is of importance to first design tools to facilitate the manual work and second to evaluate the quality of so produced `ground truth'.
More specifically, 1) a set of tools for interactive visual inspection (for RTT time series and labels) are developed to minimize human errors~\footnote{\url{https://github.com/WenqinSHAO/rtt_visual.git}}; 2) we fabricated a synthetic RTT dataset with known moments of actual change, and compared the human detection results to generated changepoints~\footnote{\url{https://github.com/WenqinSHAO/rtt_gen.git}}. The labellers are the authors of this work, who are researchers/graduate students in networking. 
...
The synthetic dataset contains 20 RTT timeseries, %with 6485 datapoints each on average,
...
These synthetic RTT timeseries are generated following these steps.
First, we randomly generate for each RTT time series several phases/segments of different delay baselines.
...
For the detailed generative models and model parameters of each step, please refer to the documentation in the code space.
An example of these synthetic RTT trace is shown in Fig.~\ref{fig:art_example}.
The red vertical lines are moments when there is a base line RTT change, or a congestion period starts/ends.
Short living spikes are considered rather as noises than real changes.
...
A great amount of real RTT traces of various characters are as well selected from RIPE Atlas to construct the ground truth dataset.
Some are full of fluctuations; some contain periodic congestion, some have many stage changes, etc.
...
%How the labelling is done.
1047 changepoints were identified by the labellers.
