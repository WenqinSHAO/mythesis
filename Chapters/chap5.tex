...
10 min is a reasonable value, given that the RTT change detection tolerance window in Section~\ref{sec:cpt_rtt} is 8 min corresponding to two measurement intervals of RIPE Atlas built-in ping measurement.
...
\paragraph{1: Common parts cause the shared event.} This assumption briefly mentioned before states that if measurements over multiple paths experience a shared RTT change, it is more likely that the common parts of these measurement path are the cause. It is not necessary true all the time, but just more likely. We use this assumption to design special sets of measurements (inference metric) for each node and link in topology graphs to test their liability at each time bin for RTT change events.

\paragraph{2: Single cause for an event.} Either for a standalone RTT change on one measurement, or for a shared event among multiple paths, there shall be no more then one causes for it. It is reasonable with AS-level topology, though don't necessary to be always true. With this assumption, we are able to distinct the contributions of node, links and neighbouring links to an RTT change event.

\subsection{Inference metric and logic}
\subsubsection{Node}
\begin{figure}[!htb]
\centering
\includegraphics[width=.9\textwidth]{gfx/chap5/dms.pdf}
\caption{Example of \acf{DMS}.}
\label{fig:chap5_dms}
\end{figure}

To exclusive verify whether a node (AS) is be the cause for shared RTT changes, we define for each node measurement sets satisfying following criteria \acf{DMS}:
\begin{itemize}
\item All the measurements pass the node in question;
\item Between any pair of measurement in the set, the only common part is the node in question.
\end{itemize}
For AS0 in Fig.~\ref{fig:chap5_dms}, it has three DMS. Each covered four divergent links and ASes other than AS0 itself. If multiple measurements within any of these DMS experience same RTT change at a certain moment, we can exclusively infer that AS0 is the cause according to the 1st assumption.

More formally, we infer that a node $n$  is the cause for shared RTT change at time bin $t$ if there exists one DMS $D$ that:
\begin{itemize}
\item $|D| >= 2$;
\item and RTT change event count for $D$ at $t$, $count(D, t)$, is larger than half of the node degree $degree(n)$.
\end{itemize}
These requires ensures a majority of measurements in $D$ experience RTT change at $t$ and there are multiple of them.
